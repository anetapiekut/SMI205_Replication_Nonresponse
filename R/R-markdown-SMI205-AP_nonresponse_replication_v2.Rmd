...
Item nonresponse in surveys is a common phenomena. Many studies demonstrated that it works differently for questions asking about knowledge and subjective-states (Berinsky 1999; Herda 2013). I reproduce some analysis conducted by Piekut (2019) to explore this matter further. Her research explores patterns in nonresponse rate to items measuring attitudes to immigration using European Social Survey (ESS) data from 2014. She analysed all nonresponses together in count models, arguing that it is a common practice in studies based on ESS to combine them together as summative indices:
...
European Social Survey is a cross-national survey conducted every two years in a number of European countries, using random probability sampling (ESS 2014). I use wave 7, collected in 2014/2015, which was also analysed by Piekut (2019), and I select the same key variables for my analysis^[There are still some dissimilarities in the final list of variables I use. I only include interviewer age and gender as controls, and I do not add any country-level variables, as I am not interested in re-testing such contextual effects]. 
...
The original paper uses also data from the interviewer questionnaire (`ESSinterviewer.sav` - SPSS  format), which I am attaching to my main dataset by matching cases by both respondents 'idno' and 'cntry'. As the country variable in the interviewer dataset is differently labelled, I have to re-label it in my ESS data, so cases in both datsets can be matched.
...
The original sample size was 40,185 respondents. After excluding Isreal sample (as Piekut (2019) did), and removing cases with missing data for the key variables it is 37,385.
...
As Tables 1 and 2 in tabs below present, almost 5.3% of respondents in the entire ESS sample did not replied to the question asking whether according to them immigration has a negative or positive impact on their country culture, while 3.3.% did not replied to the economy item. However, the proportions of nonresponse considerably vary across countries. For culture item, in Belgium only 0.8% of respondents replied 'Don't know' or 'Refuse to answer', while in Slovenia - 17.2%. Figure 1 and 2 illustrates this cross-country variation.
...
There are, however, some diffrences in the strenght of coefficients in both models. Odds ratio for women is higher in model predicting probability of nonresponse to impact on culture item. While women are 29% more likely than man to refrain from asnwering the culture item, it is 22% for nonresponse to the economy item (see Figures 3 and 4 below for comparison). On average, keeping all other variable at their means, the probability of nonresponse for women was 2% and 3%, for each item respectively.

Age effect is the same in both model, and one year results in 1% higher chances of nonrespose for each question. Since nonresponse to culture on average was higher, a probability of nonreponse for a 70-year-old person is 2.5% for economy item, and 3.5% for culture item (see Figures 5 and 6 below for comparison). 
...
We found that a few independent variables -- like the frequency of contact  -- turned out not to be statistically significant in the model with nonresponse for the immigration impact on economy item, while they remained significant in the culture nonresponse model. The proportion of nonresponse for both items was low (and just 3% for economy item), which results in a very uneven division in the dependent variable and a small number of cases in one category. Detecting patterns in distribution in unbalanced data is more difficult and logistic regression might underestimate the probability of rare events (King, Zeng 2001). The count model chosen by Piekut (2019) seems to be a sensible option trying to overcome this problem, while building on the fact that both measures relate to the same latent variable anti-immigration attitudes.
